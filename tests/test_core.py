...
class SqliteMiscTest(TestCaseBackport):
    ...
    def test_with_statement(self):
        """Verify using sqlitedict2 as a contextmanager . """
        with sqlitedict2.SqliteDict() as d:
            self.assertTrue(isinstance(d, sqlitedict2.SqliteDict))
    ...
    def test_directory_notfound(self):
        ...
        folder = tempfile.mkdtemp(prefix='sqlitedict2-test')
        os.rmdir(folder)
        # exercise,
        with self.assertRaises(RuntimeError):
            sqlitedict2.SqliteDict(filename=os.path.join(folder, 'nonexistent'))

    def test_commit_nonblocking(self):
        """Coverage for non-blocking commit."""
        # given,
        with sqlitedict2.SqliteDict(autocommit=True) as d:
...
class NamedSqliteDictCreateOrReuseTest(TempSqliteDictTest):
    ...
    def test_default_reuse_existing_flag_c(self):
        ...
        # given,
        fname = norm_file('tests/db/sqlitedict2-override-test.sqlite')
        orig_db = sqlitedict2.SqliteDict(filename=fname)
        orig_db['key'] = 'value'
        orig_db.commit()
        orig_db.close()

        next_db = sqlitedict2.SqliteDict(filename=fname)
    ...
    def test_overwrite_using_flag_n(self):
        ...
        # given,
        fname = norm_file('tests/db/sqlitedict2-override-test.sqlite')
        orig_db = sqlitedict2.SqliteDict(filename=fname, tablename='sometable')
        orig_db['key'] = 'value'
        orig_db.commit()
        orig_db.close()

        # verify,
        next_db = sqlitedict2.SqliteDict(filename=fname, tablename='sometable', flag='n')
    ...
    def test_overwrite_using_flag_w(self):
        ...
        # given,
        fname = norm_file('tests/db/sqlitedict2-override-test.sqlite')
        orig_db_1 = sqlitedict2.SqliteDict(filename=fname, tablename='one')
        orig_db_1['key'] = 'value'
        orig_db_1.commit()
        orig_db_1.close()

        orig_db_2 = sqlitedict2.SqliteDict(filename=fname, tablename='two')
        orig_db_2['key'] = 'value'
        orig_db_2.commit()
        orig_db_2.close()
        ...
        next_db_1 = sqlitedict2.SqliteDict(filename=fname, tablename='one', flag='w')
        self.assertNotIn('key', next_db_1.keys())

        next_db_2 = sqlitedict2.SqliteDict(filename=fname, tablename='two')
...
class SqliteDictTerminateTest(unittest.TestCase):
    ...
    def test_terminate_instead_close(self):
        ...
        '''
        d = sqlitedict2.open('tests/db/sqlitedict2-terminate.sqlite')
        d['abc'] = 'def'
        d.commit()
        self.assertEqual(d['abc'], 'def')
        d.terminate()
        self.assertFalse(os.path.isfile('tests/db/sqlitedict2-terminate.sqlite'))

class SqliteDictTerminateFailTest(unittest.TestCase):
    """Provide Coverage for SqliteDict.terminate()."""

    def setUp(self):
        self.fname = norm_file('tests/db-permdenied/sqlitedict2.sqlite')
        self.db = sqlitedict2.SqliteDict(filename=self.fname)
